...
class EveEntityManager(models.Manager):
    def get_or_create_from_esi(self, id: int, esi_client: object = None) -> list:
    ...
    def update_or_create_from_esi(self, id: int, esi_client: object = None) -> list:
        ...
        try:
            ...
            response = esi_client.Universe.post_universe_names(ids=[id]).result()
            if len(response) != 1:
            ...
            logger.exception(
                addPrefix("Failed to load entity with id {} from ESI: ".format(id, ex))
            )
            raise ex

        return entity, created

    def load_missing_from_esi_bulk(self, ids: list, esi_client: object = None) -> list:
        ...
        if len(unknown_ids) > 0:
            for unknown_ids_chk in chunks(list(unknown_ids), 500):
                logger.info(
                    "Fetching {} entities from ESI".format(len(unknown_ids_chk))
                )
                try:
                    if not esi_client:
                    ...
                    for id in unknown_ids_chk:
                        self.update_or_create_from_esi(id=id, esi_client=esi_client)

                except Exception as ex:
                    logger.exception(
                        "Failed to load {} entities from ESI: ".format(
                            len(unknown_ids_chk), ex
                        )
                    )
                    raise ex

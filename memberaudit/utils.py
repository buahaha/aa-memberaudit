...
class messages_plus:
    ...
    def add_messages_icon(cls, level, message):
        ...
        message = (
            '<span class="glyphicon glyphicon-{}" '.format(glyph)
    ...
    def debug(cls, request, message, extra_tags="", fail_silently=False):
        messages.debug(
            request, cls.add_messages_icon(DEBUG, message), extra_tags, fail_silently
        )

    @classmethod
    def info(cls, request, message, extra_tags="", fail_silently=False):
        messages.info(
            request, cls.add_messages_icon(INFO, message), extra_tags, fail_silently
        )

    @classmethod
    def success(cls, request, message, extra_tags="", fail_silently=False):
        messages.success(
            request, cls.add_messages_icon(SUCCESS, message), extra_tags, fail_silently
        )

    @classmethod
    def warning(cls, request, message, extra_tags="", fail_silently=False):
        messages.warning(
            request, cls.add_messages_icon(WARNING, message), extra_tags, fail_silently
        )

    @classmethod
    def error(cls, request, message, extra_tags="", fail_silently=False):
        messages.error(
            request, cls.add_messages_icon(ERROR, message), extra_tags, fail_silently
        )

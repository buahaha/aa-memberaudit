...
def _get_token_for_owner(owner: Owner, add_prefix: make_logger_prefix) -> list:
    ...
    if not owner.character.user.has_perm("memberaudit.basic_access"):
...
def sync_mailinglists(owner: Owner, esi_client: object):
    add_prefix = make_logger_prefix(owner)

    mailing_lists = esi_client.Mail.get_characters_character_id_mail_lists(
        character_id=owner.character.character.character_id
    ).result()
...
def sync_mails(owner: Owner, esi_client: object, add_prefix: make_logger_prefix):
    ...
    while True:
        logger.info(add_prefix("Fetching mail headers from ESI - page {}".format(page)))
        mail_headers = esi_client.Mail.get_characters_character_id_mail(
            character_id=owner.character.character.character_id,
    ...
    logger.info(
        add_prefix("Received {} mail headers from ESI".format(len(mail_headers_all)))
    )

    if settings.DEBUG:
        # store to disk (for debugging)
        with open(
            "mail_headers_raw_{}.json".format(owner.character.character.character_id),
            "w",
            encoding="utf-8",
        ) as f:
            json.dump(
    ...
    mailing_list_ids = [
        x["list_id"]
        for x in MailingList.objects.filter(owner=owner)
        .select_related()
        .values("list_id")
    ]
    ...
    for header in mail_headers_all:
        try:
            with transaction.atomic():
                try:
                    from_mailing_list = MailingList.objects.get(list_id=header["from"])
                    from_entity = None
                except MailingList.DoesNotExist:
                    from_entity, _ = EveEntity.objects.get_or_create_from_esi(
                        header["from"], esi_client
                    )
                ...
                for recipient in header["recipients"]:
                    if recipient["recipient_type"] != "mailing_list":
                        recipient, _ = EveEntity.objects.get_or_create_from_esi(
                            recipient["recipient_id"], esi_client
                        )
                        MailRecipient.objects.create(mail=mail_obj, recipient=recipient)
                ...
                if mail_obj.body is None:
                    ...
                    mail = esi_client.Mail.get_characters_character_id_mail_mail_id(
                        character_id=owner.character.character.character_id,

...
class TestViews(TestCase):
    ...
    def test_character_mails_data(self):
        ...
        request = self.factory.get(
            reverse("memberaudit:character_mails_data", args=[self.character.pk])
        )
        request.user = self.user
        response = character_mails_data(request, self.character.pk)
        self.assertEqual(response.status_code, 200)
        data = json_response_to_python(response)
        self.assertEqual(len(data), 1)
        row = data[0]
        ...
        self.assertListEqual(row["to"], ["Bruce Wayne", "Mailing List"])
